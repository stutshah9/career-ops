...
- **`output/stuti-shah-cv.tex`** — full Jake-LaTeX master (all experiences + projects + skills), rebuilt from `cv.md`; this is now the source parsed by `scripts/tailor-resume.mjs`. User may edit it; don't delete without asking. (Was accidentally deleted once on 2026-05-31, then rebuilt.)
...
- **CV format is HTML.** `output/stuti-shah-cv.{html,pdf}` is the generic master (HTML render, ~116 KB). `output/stuti-shah-cv.tex` is a full Jake-LaTeX master rebuilt from `cv.md` and is now parsed by the HTML resume generator; don't delete without asking.
...
| `output/stuti-shah-cv.{html,pdf}` | Generic master CV (HTML render) |
